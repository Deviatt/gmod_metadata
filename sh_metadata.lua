...
local stored, pcache, Sync, encode = metadata.stored, {} do
	...
	local net = net
	if (SERVER) then
		util.AddNetworkString("Metadata::Sync")
...
hook.Add("Initialize", "Metadata::Init", function()
	sql.Query[[CREATE TABLE IF NOT EXISTS `metadata`(
	`sid64` CHAR(17) NOT NULL,
	`key` TINYTEXT,
	`value` BIGTEXT,
	`type` TINYINT)]]
	hook.Remove("Initialize", "Metadata::Init")
end)
...
hook.Add("PlayerInitialSpawn", "Metadata::Sync", function(ply)
	local sid64 = ply:SteamID64()
...
hook.Add("PlayerDisconnected", "Metadata::Clear", function(ply)
